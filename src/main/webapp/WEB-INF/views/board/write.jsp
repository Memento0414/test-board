...
<body>
	<form action="/board/write-task" method="post" class="form-write">
		<fieldset>
			<legend>글등록</legend>
			<div>
				<c:choose>
					<c:when test="${sessionScope.logon eq true }">
						<input type="text" name="userId" placeholder="작성자"/>
					...
					<c:otherwise>
						...
						<input type="password" name="userPass" id="userInfo"placeholder="비밀번호"/>
						</label>
						<input type="text" name="title" placeholder="제목을 입력해주세요"/>
